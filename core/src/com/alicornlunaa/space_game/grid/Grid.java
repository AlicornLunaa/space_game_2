...
import com.alicornlunaa.selene_engine.phys.Collider.EdgeShape;
...
public class Grid {
    ...
    public void assemble(BodyComponent bodyComponent){
        ...
        // Hull for other things colliding with this grid
        ...
        Collider edgeCollider = new Collider();
        Vector2 plyPos = new Vector2(0, 0);
        ...
        for(int y = loadDist * -1 * Constants.CHUNK_SIZE; y < loadDist * Constants.CHUNK_SIZE; y++){
            for(int x = loadDist * -1 * Constants.CHUNK_SIZE; x < loadDist * Constants.CHUNK_SIZE; x++){
                ...
                AbstractTile tile = getTile(x + plyTileX, y + plyTileY, Layer.MIDDLE);
                AbstractTile tileAbove = getTile(x + plyTileX, y + plyTileY + 1, Layer.MIDDLE);
                AbstractTile tileBelow = getTile(x + plyTileX, y + plyTileY - 1, Layer.MIDDLE);
                ...
                if(tile != null && tileAbove == null && startTop == -1){
                    ...
                    EdgeShape edge = edgeCollider.addEdgeShape();
                    edge.set(
        ...
        for(int x = loadDist * -1 * Constants.CHUNK_SIZE; x < loadDist * Constants.CHUNK_SIZE; x++){
            for(int y = loadDist * -1 * Constants.CHUNK_SIZE; y < loadDist * Constants.CHUNK_SIZE; y++){
                // Get the chunk coordinates and tile coordinates from the global x and y
                AbstractTile tile = getTile(x + plyTileX, y + plyTileY, Layer.MIDDLE);
                AbstractTile tileLeft = getTile(x - 1 + plyTileX, y + plyTileY, Layer.MIDDLE);
                AbstractTile tileRight = getTile(x + 1 + plyTileX, y + plyTileY, Layer.MIDDLE);
        ...
        // bodyComponent.addCollider(edgeCollider);
    }
