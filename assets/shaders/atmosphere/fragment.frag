...
float densityAtPoint(vec3 sample){
    float heightAboveSurface = length(sample - vec3(u_planetWorldPos, 0.0)) - u_planetRadius;
...
vec3 light(vec3 rayOrigin, vec3 rayDir, float rayLength){
    ...
    for(int i = 0; i < MAX_IN_SCATTER_POINTS; i++){
        float sunRayLength = sphereRaycast(vec3(u_planetWorldPos, 0.0), u_atmosRadius, inScatterPoint, u_starDirection).y;
...
void main() {
    ...
    vec2 atmosRay = sphereRaycast(vec3(u_planetWorldPos, 0.0), u_atmosRadius, rayOrigin, rayDir);
